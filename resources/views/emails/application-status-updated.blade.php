...
<head>
    ...
    <title>Status Lamaran SIKARA</title>
    <style>
        ...
        .status { background: #eff6ff; border-left: 4px solid #2563eb; border-radius: 6px; padding: 18px; margin: 24px 0; color: #1e3a8a; font-weight: 700; }
...
<div class="wrapper">
    ...
    <div class="body">
        ...
        <p>Lamaran Anda untuk posisi <strong>{{ $internship?->title ?? 'Lowongan magang' }}</strong> di <strong>{{ $internship?->company_name ?? 'Perusahaan mitra' }}</strong> telah diperbarui.</p>
        <div class="status">Status saat ini: {{ $statusLabel }}</div>
        <p>{{ $guidance }}</p>
        <div class="btn-wrap">
            <a href="{{ $url }}" class="btn">Lihat Lamaran Saya</a>
        </div>
    </div>
    <div class="footer">
